...
require_once($CFG->dirroot.'/phrasecomment/locallib.php');
...
$PAGE->requires->js_init_call('M.core_phrasecomment.init_admin', null, true,$block_phrasecommentjs);
...
if ($action === 'delete') {
    ...
    if (!empty($commentid)) {
        if (!$confirm) {
            ...
            $buttoncontinue = new single_button(new moodle_url('/phrasecomment/index.php', $optionsyes), get_string('delete'));
            $buttoncancel = new single_button(new moodle_url('/phrasecomment/index.php', $optionsno), get_string('cancel'));
            ...
            if ($manager->delete_comment($commentid)) {
                redirect($CFG->httpswwwroot.'/phrasecomment/');
            } else {
...
if (!empty($err)) {
    print_error($err, 'error', $CFG->httpswwwroot.'/phrasecomment/');
}
if (empty($action)) {
